...
(function() {
    ...
    var kAllowedOptions = ["collection", "modelView", "modelViewOptions", "itemTemplate", "emptyListCaption", "selectable", "clickToSelect", "selectableModelsFilter", "visibleModelsFilter", "selectMultiple", "clickToToggle", "processKeyEvents", "sortable", "sortableModelsFilter"];
    ...
    Backbone.CollectionView = Backbone.View.extend({
            ...
            if (name === "collection") {
                ...
                if (_.contains(kAllowedOptions, name)) {
                    ...
                    switch (name) {
                        case "selectMultiple":
                            this[name] = value;
                            if (!value && this.selectedItems.length > 1) this.setSelectedModel(_.first(this.selectedItems), {
                                by: "cid"
                            });
                            break;
                        case "selectable":
                            if (!value && this.selectedItems.length > 0) this.setSelectedModels([]);
                            this[name] = value;
                            break;
                        case "selectableModelsFilter":
                            this[name] = value;
                            if (value && _.isFunction(value)) this._validateSelection();
                            break;
                        case "itemTemplate":
                            this[name] = value;
                            this._updateItemTemplate();
        ...
        getSelectedModels: function(options) {
            ...
            switch (referenceBy) {
                ...
                case "offset":
                    ...
                    var itemElements;
                    if (this._isRenderedAsTable()) itemElements = this.$el.find("> tbody > [data-item-id]:visible");
                    else if (this._isRenderedAsList()) itemElements = this.$el.find("> [data-item-id]:visible");
        ...
        setSelectedModel: function(newSelectedItem, options) {
            ...
            else this.setSelectedModels([newSelectedItem], options);
        },
        ...
        render: function() {
            ...
            if (this.itemTemplateFunction != null) {
                ...
                if (this._isRenderedAsTable() && tbodyChild.length > 0) modelViewParentElement = $(tbodyChild[0]);
                else modelViewParentElement = this.$el;
            ...
            if (!_.isNull(this.emptyListCaption)) {
                ...
                if (_.isUndefined(visibleView)) {
                    ...
                    if (_.isFunction(this.emptyListCaption)) emptyListString = this.emptyListCaption();
                    else emptyListString = this.emptyListCaption;
                    ...
                    if (this._isRenderedAsList()) $emptyListCaptionEl = $varEl.wrapAll("<li></li>").parent().css(kStylesForEmptyListCaption);
                    else $emptyListCaptionEl = $varEl.wrapAll("<tr><td></td></tr>").parent().parent().css(kStylesForEmptyListCaption);
        ...
        _onKeydown: function(event) {
            ...
            if (this.getSelectedModels({
                ...
                if (event.which == this._charCodes.upArrow) {
                    this.setSelectedModel(this.getSelectedModel({
                        by: "offset"
                    }) - 1, {
                        by: "offset"
                    });
